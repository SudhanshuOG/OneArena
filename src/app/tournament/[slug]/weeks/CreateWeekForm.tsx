...
export default function CreateWeekForm({ serverTournamentId, serverSlug }: Props) {
  ...
  return (
    <form onSubmit={handleCreate} className="flex gap-2 items-center">
      <input
        ...
        min={1}
        value={weekNo}
        onChange={(e) => setWeekNo(e.target.value === '' ? '' : Number(e.target.value))}
      ...
      <input
        ...
        className="px-2 py-1 rounded bg-zinc-900"
      />
      <button type="submit" disabled={loading} className="px-3 py-1 rounded bg-sky-600">
